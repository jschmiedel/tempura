...
#' @param abundancepca_files absolute path to the DiMSum-processed abundancePCA .RData files containing aa_seq, fitness and sigma cols; multiple files will be treated as independent measurements
#' @param bindingpca_files absolute path to the DiMSum-processed bindingPCA .RData files containing aa_seq, fitness and sigma cols; multiple files will be used as independent measurements
#' @param wt_seq a character string of the wild-type/reference aa sequence
...
dg_prepare_datasets = function(
	...
	for (i in 2:length(all_files)) {
		variant_data <- merge(variant_data, all_files[[i]], by = "aa_seq", all = T)
	}
